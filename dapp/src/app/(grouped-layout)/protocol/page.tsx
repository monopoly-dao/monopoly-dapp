...
const architectureHref =
  'https://github.com/monopoly-dao/monopoly-dapp/blob/codex/frontend-cleanup-settley/docs/settley-stellar-technical-architecture.md';
    ...
    <div className='bg-settley-bg text-navy'>
      <section className='px-[5%] lg:px-[7%] py-20 sm:py-28 lg:py-36 border-b border-[#C6C6CD]/20'>
        <div className='max-w-5xl'>
          ...
          <p className='mt-8 font-inter text-2xl sm:text-3xl max-w-4xl text-navy'>
            Conditional liquidity infrastructure for illiquid real-world assets.
          </p>
          <p className='mt-10 font-inter text-lg sm:text-xl max-w-4xl text-[#3B3C4A]'>
            Tokenization solved issuance. Settley solves the liquidity
            bottleneck.
          </p>
          <p className='mt-5 font-inter text-lg sm:text-xl max-w-4xl text-[#3B3C4A]'>
          ...
          <div className='mt-10 flex flex-col sm:flex-row gap-4'>
            <Link
              href={whitepaperHref}
              target='_blank'
              className='bg-navy text-white rounded-full py-4 px-6 w-fit font-inter font-medium flex items-center gap-3 hover:bg-navy/90 transition'
            >
              <FileText size={18} />
              Read the Whitepaper
            </Link>
            <Link
              ...
              target='_blank'
              className='border border-navy text-navy rounded-full py-4 px-6 w-fit font-inter font-medium flex items-center gap-3 hover:bg-navy/5 transition'
            >
              <GitBranch size={18} />
      ...
      <section className='px-[5%] lg:px-[7%] py-16 sm:py-24 bg-white'>
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20'>
          ...
          <p className='font-inter text-lg text-[#3B3C4A] leading-relaxed'>
      ...
      <section className='px-[5%] lg:px-[7%] py-16 sm:py-24'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-5'>
          {mechanisms.map((item) => (
            <div
              key={item.title}
              className='border border-[#C6C6CD]/20 rounded-3xl p-6 bg-white min-h-[220px] shadow-sm'
            >
              <h3 className='font-playfair text-xl font-bold text-navy'>
                {item.title}
              </h3>
              <p className='mt-5 font-inter text-[#3B3C4A]'>{item.copy}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className='max-w-4xl'>
          ...
          <div className='mt-8 grid grid-cols-1 gap-4'>
            {statusItems.map((item, index) => (
              <div
                key={item}
                className='border border-[#C6C6CD]/20 bg-settley-bg rounded-3xl p-5 flex gap-4 items-start'
              >
                <span className='h-8 w-8 shrink-0 rounded-full bg-navy text-white flex items-center justify-center text-sm font-medium'>
                  {index + 1}
                </span>
                <p className='font-inter text-navy font-medium leading-relaxed'>
                  {item}
                </p>
              </div>
            ))}
